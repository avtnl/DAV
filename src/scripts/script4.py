...
from src.plot_manager import RelationshipsPlotSettings
...
class Script4(BaseScript):
    """Relationships plot across multiple groups."""
    ...
    def __init__(
        ...
        image_dir: Path,
        df: pd.DataFrame,
        settings: RelationshipsPlotSettings | None = None,
    ) -> None:
        """
        ...
        Args:
            ...
            data_preparation: DataPreparation for relationships data.
            plot_manager: PlotManager for rendering.
            image_dir: Directory to save plot.
            df: Enriched DataFrame (required).
            settings: Relationships plot settings (optional).
        """
        super().__init__(
            file_manager=file_manager,
            data_preparation=data_preparation,
            plot_manager=plot_manager,
            settings=settings or RelationshipsPlotSettings(),
            df=df,
        )
    ...
    def run(self) -> Path | None:
        """
        Generate and save the words vs punctuation relationships (bubble) plot.
        ...
        bubble_data = self.data_preparation.build_visual_relationships(self.df)
        if bubble_data is None:
            self.log_error("build_visual_relationships returned None.")
            return None

        logger.info(f"Relationships plot data: {len(bubble_data.feature_df)} author-group rows")

        fig = self.plot_manager.build_visual_relationships(
            bubble_data,
            self.settings
        )
        if fig is None:
