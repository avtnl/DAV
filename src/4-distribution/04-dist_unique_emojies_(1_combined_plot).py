...
def main():
    ...
    fig, ax = plt.subplots(figsize=(8, len(emoji_counts_top) * 0.4))
    y_positions = np.arange(len(emoji_counts_top))  # Evenly spaced y-positions
    bar_width = 0.4  # Width for each bar (two bars per emoji)

    # Plot count_all bars (gray)
    ax.barh(y_positions - bar_width / 2, emoji_counts_top["count_all"], height=bar_width, color="#808080", label="All Occurrences")
    
    # Plot count_once bars (light blue)
    ax.barh(y_positions + bar_width / 2, emoji_counts_top["count_once"], height=bar_width, color="#ADD8E6", label="Once per Message")
    
    # Customize plot
    ax.set_yticks(y_positions)
    ax.set_yticklabels(emoji_counts_top["emoji"])
    ax.set_xlabel("Count")
