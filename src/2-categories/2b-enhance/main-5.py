...
def main():
    ...
    yearly_counts = all_years.merge(
        yearly_counts,
        on="year",
        how="left"
    ).fillna({"message_count": 0, "whatsapp_group": yearly_counts["whatsapp_group"].mode().iloc[0] if not yearly_counts.empty else "Unknown", "author": "Unknown"})
    yearly_counts["message_count"] = yearly_counts["message_count"].astype(int)
    logger.debug(f"Yearly counts head (with all years):\n{yearly_counts.head().to_string()}")

    fig, ax = plt.subplots(figsize=(12, 6))

    non_anthony_authors = [auth for auth in df["author"].unique() if auth != "Anthony van Tilburg" and auth != "Unknown"]
    ...
    for author in non_anthony_data["author"].unique():
        ...
        author_data = all_years.merge(author_data, on="year", how="left").fillna({"message_count": 0})
    ...
    for group in df["whatsapp_group"].unique():
        ...
        group_avg = group_non_anthony.groupby(["whatsapp_group", "year"])["message_count"].mean().reset_index(name="group_avg")
        group_avg = all_years.merge(group_avg, on="year", how="left").fillna({"group_avg": 0, "whatsapp_group": group})
        ax.plot(group_avg["year"], group_avg["group_avg"], 
                color="gray", linestyle="--", linewidth=1.5, label=f"Avg {group} (Non-Anthony)")
    ...
    for group in df["whatsapp_group"].unique():
        ...
        group_anthony = all_years.merge(group_anthony, on="year", how="left").fillna({"message_count": 0, "whatsapp_group": group, "author": "Anthony van Tilburg"})
        if group_anthony["message_count"].sum() > 0:
            ax.plot(group_anthony["year"], group_anthony["message_count"], 
                    color="darkgray", linewidth=1.5, label=f"Anthony ({group})")

    # Plot overall average (non-Anthony, black)
    overall_avg = non_anthony_data.groupby("year")["message_count"].mean().reset_index(name="overall_avg")
    overall_avg = all_years.merge(overall_avg, on="year", how="left").fillna({"overall_avg": 0})
    ax.plot(overall_avg["year"], overall_avg["overall_avg"], 
            color="black", linewidth=2, label="Overall Avg (Non-Anthony)")

    # Plot overall Anthony average (blue)
    overall_anthony = anthony_data.groupby("year")["message_count"].mean().reset_index(name="anthony_avg")
    overall_anthony = all_years.merge(overall_anthony, on="year", how="left").fillna({"anthony_avg": 0})
    if overall_anthony["anthony_avg"].sum() > 0:
        ax.plot(overall_anthony["year"], overall_anthony["anthony_avg"], 
                color="blue", linewidth=2, label="Anthony Avg (All Groups)")
    ...
    # Additional bar chart for July 2020 - July 2025
    ...
    # Calculate average messages per author (non-Anthony) per group
    ...
    non_anthony_group = non_anthony_group.set_index('whatsapp_group').reindex(sorted_groups).reset_index().fillna(0)
    logger.info(f"Non-Anthony average messages per group:\n{non_anthony_group.to_string()}")
    ...
    anthony_group = anthony_group.set_index('whatsapp_group').reindex(sorted_groups).reset_index().fillna(0)
    ...
    # Black bars for non-Anthony average
    ax.bar(positions, non_anthony_group['non_anthony_avg'], width=bar_width, color='black', label='Avg Non-Anthony per Author')
    # Red bars overlapping 50% to the right
    ax.bar(positions + bar_width / 2, anthony_group['anthony_messages'], width=bar_width, color='red', label='Anthony van Tilburg (Total)')
    # Customize plot
    ...
    ax.set_title("Participation of Anthony in WA-group `maap´ is significant lower than other WA-groups")
    ax.legend()
    plt.tight_layout()
