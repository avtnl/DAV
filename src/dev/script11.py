...
class Script11(BaseScript):
    ...
    def _check_input(
        ...
        plot_feed: str,
        groupby_period: str,
        delete_specific_attributes: bool,
        plot_type: str,
    ) -> bool:
        ...
        if groupby_period not in ["week", "month", "year"]:
            ...
            return False
        if delete_specific_attributes and (
            data_feed != "non_redundant" or groupby_period != "month"
        ):
    ...
    # ------------------------------------------------------------------ #
    def _load_and_preprocess(self, data_feed: str) -> pd.DataFrame | None:
        prefix = (
            "organized_data_no_redundancy"
            if data_feed == "non_redundant"
            else "organized_data_with_redundancy"
        )
    ...
    # ------------------------------------------------------------------ #
    def _delete_and_save(
        self, df: pd.DataFrame, delete_specific_attributes: bool
    ) -> pd.DataFrame | None:
        ...
        csv = self.file_manager.save_csv(df, self.processed_dir, prefix="organized_data_specific")
        parq = self.file_manager.save_parq(df, self.processed_dir, prefix="organized_data_specific")
    ...
    def run(self):
        try:
            ...
            if not self._check_input(data_feed, plot_feed, groupby, delete_attr, plot_type):
                return None
