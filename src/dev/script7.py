...
class Script7(BaseScript):
    ...
    def run(self):
        df_filtered = self.data_preparation.df[
            self.data_preparation.df["whatsapp_group"] != "tillies"
        ].copy()
        ...
        feature_df = self.data_preparation.build_interaction_features(
            df_filtered, group_authors_filtered
        )
