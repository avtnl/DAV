...
def render_category_tab(df_filtered) -> None:
    ...
    if view_mode == "whatsapp_group":
        ...
        non_avt = non_avt.sort_values([COL["group"], "count"], ascending=[True, False]).reset_index(
            drop=True
        )
        ...
        fig.update_xaxes(
            ...
            ticktext=[
                x.split("<br>")[-1]
                for x in author_group["x_label"]
                if not x.startswith("__spacer__")
            ],
        ...
        st.caption(
            "Authors ordered by message volume within each group. AvT shown last. Groups separated for clarity."
        )
    ...
    # ------------------------------------------------------------------
        ...
        total_sorted = (
            pd.concat([non_avt, avt_row], ignore_index=True) if not avt_row.empty else non_avt
        )
        ...
        fig_total.update_traces(
            textposition="outside",
            marker_line={"width": 1.5, "color": "black"},
        )
