# --------------------------------------------------------------
# style_analyzer_gen3.py  (FINAL – WITH whatsapp_group_temp FROM ENRICH)
# --------------------------------------------------------------
import pandas as pd
import numpy as np
...
# ------------------------------------------------------------------
# NEW: Constant for group column created in run_enrich.py
# ------------------------------------------------------------------
...
def load_enriched() -> pd.DataFrame:
    ...
    # Verify required columns (including whatsapp_group_temp from enrich)
    required = [
...
def aggregate_basic(df: pd.DataFrame, group_cols: list[str]) -> pd.DataFrame:
    # Include whatsapp_group_temp in groupby
    ...
    burst_defs = [
        ...
        (Columns.LIST_OF_CONNECTED_EMOJIS.value, {
            ...
            '🤣🤣': 'laugh_2', '🤣🤣🤣': 'laugh_3plus',
            '😄😄': 'happy_2',
            'thumbs_up thumbs_up': 'thumbs_2', 'thumbs_up_light_skin_tone thumbs_up_light_skin_tone': 'thumbs_2',
            'thumbs_up_medium-light_skin_tone thumbs_up_medium-light_skin_tone': 'thumbs_2',
            'heart': 'heart_single',
        }),
...
# ------------------------------------------------------------------
# 8. Individual t-SNE – WITH draw_ellipses OPTION
# ------------------------------------------------------------------
def plot_tsne_individual(agg: pd.DataFrame, out_dir: Path, draw_ellipses: bool = True):
    ...
    fig = px.scatter(
        ...
        title="Linguistic Style Clusters (t-SNE) – Individual Authors – Conf 50%",
    ...
    # === OPTIONAL: Draw 50% confidence ellipses ===
    if draw_ellipses:
        for author in agg[Columns.AUTHOR.value].unique():
            ...
            fig.add_annotation(
                x=mean_x, y=mean_y, text=author, showarrow=False,
...
# ------------------------------------------------------------------
# 9. Group-level t-SNE – WITH draw_ellipses OPTION
# ------------------------------------------------------------------
def plot_tsne_group(agg: pd.DataFrame, out_dir: Path, draw_ellipses: bool = True):
    out_dir.mkdir(parents=True, exist_ok=True)

    # --- CREATE 5 GROUPS: AvT is a separate group ---
    def assign_group(row):
        if row[Columns.AUTHOR.value] == "AvT":
            return "AvT"
    ...
    if draw_ellipses:
        for grp in ["maap", "dac", "golfmaten", "tillies", "AvT"]:
            ...
            fig.add_annotation(
                x=mean_x, y=mean_y, text=grp, showarrow=False,
...
## ------------------------------------------------------------------
# 10. Full pipeline – WITH draw_ellipses OPTION
# ------------------------------------------------------------------
def run_style_analysis(
    by_year: bool = True,
    do_clustering: bool = True,
    by_group: bool = False,
    draw_ellipses: bool = True,  # ← NEW: control ellipses
...
if __name__ == "__main__":
    ...
    agg = run_style_analysis(by_year=True, by_group=False, draw_ellipses=True)  # Group view
