...
class DataEditor:
    ...
    def convert_timestamp(self, datafile):
        try:
            logger.debug(f"Parsing raw file: {datafile}")
            # Read first few lines for debugging
            with open(datafile, 'r', encoding='utf-8') as f:
                sample_lines = f.readlines()[:5]
                logger.debug(f"Sample lines from {datafile}:\n{''.join(sample_lines)}")
            
            # Parse WhatsApp chat log
            df = pd.read_csv(datafile, names=['raw'], sep='^', engine='python', encoding='utf-8', quoting=csv.QUOTE_NONE)
            # Extract timestamp and author_message
            df[['timestamp', 'author_message']] = df['raw'].str.extract(r'\[(\d{2}-\d{2}-\d{4}, \d{2}:\d{2}:\d{2})\] (.+)', expand=True)
            df = df.dropna(subset=['timestamp', 'author_message'])
            if df.empty:
                logger.error(f"No valid data parsed from {datafile}")
                return None
            
            # Split author_message into author and message
            df[['author', 'message']] = df['author_message'].str.split(': ', n=1, expand=True)
            # Handle system messages (no colon separator)
            df.loc[df['message'].isna(), 'message'] = df['author_message']
            df.loc[df['message'].isna(), 'author'] = df['author'].fillna('System')
            df = df[['timestamp', 'author', 'message']]
            # Convert timestamp to datetime
            df['timestamp'] = pd.to_datetime(df['timestamp'], format='%d-%m-%Y, %H:%M:%S', errors='coerce')
            if df['timestamp'].isna().all():
                logger.error(f"Failed to parse timestamps in {datafile}")
                return None
            # Assign default group
            df['whatsapp_group'] = Groups.UNKNOWN.value
            logger.debug(f"Parsed DataFrame from {datafile}: shape={df.shape}, columns={df.columns.tolist()}")
            logger.debug(f"Parsed DataFrame head:\n{df.head().to_string()}")
            return df
        except Exception as e:
            logger.exception(f"Failed to convert timestamp for {datafile}: {e}")
            return None
    ...
    def list_of_all_emojis(self, text):
        if not isinstance(text, str):
            return []
        return [char for char in text if char in emoji.EMOJI_DATA and char not in self.ignore_emojis]
    ...
    def get_day_of_week(self, df):
        if 'timestamp' not in df.columns:
            logger.error("No 'timestamp' column in DataFrame")
            return None
        return df['timestamp'].dt.day_name()
    ...
    def mark_more_early_leavers(self, df):
        early_authors = ['Luka', 'Melissa', 'Wil', '+31 6 46661140‬']
        df.loc[df['author'].isin(early_authors), 'early_leaver'] = True
        return df
    ...
    def organize_extended_df_(self, df):
        """
