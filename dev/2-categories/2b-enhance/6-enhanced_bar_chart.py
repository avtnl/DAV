...
def main():
    ...
    ax.bar(positions, non_anthony_group['non_anthony_avg'], width=bar_width, color='lightgray', label='Average Non-Anthony')

    # Red bars overlapping 50% to the right
    ax.bar(positions + bar_width / 2, anthony_group['anthony_messages'], width=bar_width, color='blue', label='Anthony')

    # Add red arrow for maap
    maap_idx = sorted_groups.index('maap') if 'maap' in sorted_groups else None
    if maap_idx is not None:
        # x_pos = positions[maap_idx] + bar_width / 2  # Upper middle of light gray bar
    ...
    ax.set_title("Anthony's participation in whatsapp_group 'maap' is significant lower than other groups")
    ax.legend()
    plt.tight_layout()
