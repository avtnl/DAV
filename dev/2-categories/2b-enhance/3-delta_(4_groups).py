...
def main() -> None:
    ...
    yearly_counts = all_groups_years.merge(
        yearly_counts, on=["whatsapp_group", "year"], how="left"
    ...
    for group in df["whatsapp_group"].unique():
        ...
        non_anthony_authors = [
            auth for auth in group_authors[group] if auth not in {"Anthony van Tilburg", "Unknown"}
        ]
        ...
        avg_yearly = all_years.merge(avg_yearly, on="year", how="left").fillna(
            {"avg_message_count": 0, "whatsapp_group": group}
        )
        ...
        anthony_data = all_years.merge(anthony_data, on="year", how="left").fillna(
