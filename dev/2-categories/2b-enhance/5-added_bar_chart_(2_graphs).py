...
def main() -> None:
    ...
    non_anthony_group = (
        non_anthony_group.set_index("whatsapp_group").reindex(sorted_groups).reset_index().fillna(0)
    )
    ...
    anthony_group = (
        anthony_group.set_index("whatsapp_group").reindex(sorted_groups).reset_index().fillna(0)
    )
    ...
    ax.bar(
        ...
        width=bar_width,
        color="black",
        label="Avg Non-Anthony per Author",
    )
    ...
    ax.bar(
        ...
        width=bar_width,
        color="red",
        label="Anthony van Tilburg (Total)",
    )
    # Customize plot
    ...
    ax.set_title(
        "Participation of Anthony in WA-group `maap´ is significant lower than other WA-groups"
    )
    ax.legend()
    plt.tight_layout()
