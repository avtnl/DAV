...
def main():
    ...
    df["year-week"] = df["timestamp"].dt.strftime("%Y-%W")  # Kept for compatibility
    logger.info(df.head())

    # Reindex in order to fill the missing months
    df = df.drop(index=[0])
    ...
    average_data = p.groupby("month")["count"].mean().reset_index(name="avg_count")
    ...
    output_path = Path("img/messages_by_month_per_year_with_average.png")
