...
def main():
    ...
    # Scale sunset times to message counts
    sun_set_times = ["16:45", "17:45", "18:30", "20:15", "21:15", "21:45", "21:30", "21:00", "20:00", "18:45", "16:45", "16:30"]
    week_ticks = [1, 5, 9, 14, 18, 23, 27, 31, 36, 40, 44, 49]
    # Convert HH:MM to minutes
    def to_minutes(time_str):
        h, m = map(int, time_str.split(":"))
        return h * 60 + m
    sun_set_minutes = [to_minutes(t) for t in sun_set_times]
    # Linear mapping: 16:30 (990 min) = 2 messages, 22:00 (1320 min) = 20 messages
    # m = (3/55) * (t - 990) + 2
    sun_set_counts = [(3/55) * (t - 990) + 2 for t in sun_set_minutes]
    ...
    # Plot 2020 data in red
    data_2020 = p[p["year"] == 2020]
    if not data_2020.empty:
        sns.lineplot(
            ...
            label="2020",
            linewidth=2.5,
            zorder=3,  # Same level as blue line
        )

    # Plot yellow sun symbols for sunset times
    ax.scatter(
        x=week_ticks,
        y=sun_set_counts,
        marker="o",  # Circle as fallback (☀ may not render in all systems)
        color="yellow",
        edgecolors="black",  # Black outline for visibility
        s=100,  # Size of the marker
        zorder=6,  # Above all lines
        # No label to exclude from legend
    )
    ...
    ax.set_xlabel("Week of Year / Month")
    ax.set_ylabel("Message Count")
    plt.title("Our weekly golf tournament follows the sun")

    # Ensure legend includes only data lines
    ax.legend(title="Averages")

    output_path = Path("img/average_messages_by_week_with_2020_and_sun_symbols.png")
