...
def main() -> None:
    ...
    # Plotting count_once
    _fig, ax = plt.subplots(figsize=(len(emoji_counts_df) * 0.3, 8))  # Increased height for table
    ax2 = ax.twinx()  # Secondary y-axis for cumulative percentage
    x_positions = np.arange(len(emoji_counts_df))
    ax.bar(x_positions, emoji_counts_df["percent_once"], color="#ADD8E6", align="edge", width=0.5)
    ...
    ax2.set_title("Top 25:", loc="center", y=0.1, fontsize=12)  # Adjusted title position
