...
def main() -> None:
    ...
    for message in emoji_msgs["message"]:
        ...
        emojis = [
            char for char in message if char in emoji.EMOJI_DATA and char not in ignore_emojis
        ]
    ...
    ax1.set_ylim(ylim_bottom - 3, ylim_top)
    # Calculate x for 75%
    cum_all_np = np.array(cumulative_all)
    idx_all = np.where(cum_all_np >= 75)[0][0]
    x_all = idx_all + 1
    y_all = len(emoji_counts_df)
    # Add vertical dashed line
    ax1.axvline(x=idx_all + 0.5, color="orange", linestyle="--", linewidth=1)
    # Add texts for x and y emojis
    left_mid = idx_all / 2
    right_mid = (idx_all + 0.5) + (y_all - idx_all - 1) / 2
    y_text = ylim_bottom - 1.5
    ax1.text(left_mid, y_text, f"{x_all} emojis", ha="center", fontsize=8)
    ax1.text(right_mid, y_text, f"{y_all} emojis", ha="center", fontsize=8)
    ax1.set_xticks([])  # No x-ticks
    ...
    # Add table for top 25 emojis (count_all), with rank first
    top_25_all = emoji_counts_df.head(25)
    cum_all_top = top_25_all["percent_all"].cumsum()
    table_data = [
        [str(i + 1) for i in range(25)],
        list(top_25_all["emoji"]),
        [f"{count:.0f}" for count in top_25_all["count_all"]],
        [f"{cum:.1f}%" for cum in cum_all_top],
    ]
    table = ax1.table(
        cellText=table_data,
    ...
    ax2.set_ylim(ylim_bottom - 3, ylim_top)
    # Calculate x for 75%
    ...
    # Add vertical dashed line
    ax2.axvline(x=idx_once + 0.5, color="orange", linestyle="--", linewidth=1)
    # Add texts for x and y emojis
    left_mid = idx_once / 2
    right_mid = (idx_once + 0.5) + (y_once - idx_once - 1) / 2
    y_text = ylim_bottom - 1.5
    ax2.text(left_mid, y_text, f"{x_once} emojis", ha="center", fontsize=8)
    ax2.text(right_mid, y_text, f"{y_once} emojis", ha="center", fontsize=8)
    ax2.set_xticks([])  # No x-ticks
    # Plot cumulative line and 75% dashed line
