...
def main() -> None:
    ...
    emoji_counts_df = pd.DataFrame(
        {"emoji": list(count_once.keys()), "count_once": list(count_once.values())}
    )
    ...
    # Plotting count_once
    fig, ax = plt.subplots(figsize=(len(emoji_counts_df) * 0.3, 8))  # Increased height for table
    ax2 = ax.twinx()  # Secondary y-axis for cumulative percentage
    x_positions = np.arange(len(emoji_counts_df))
    ax.bar(x_positions, emoji_counts_df["percent_once"], color="purple", align="edge", width=0.5)
    ax.set_ylabel(
        "Likelyhood (%) of finding an Emoji in a random chosen message", fontsize=12, labelpad=20
    )
    ...
    ax.text(left_mid, y_text, f"<-- {x_once} emojies -->", ha="center", fontsize=12)
    ax.text(right_mid, y_text, f"<-- {y_once} emojies -->", ha="center", fontsize=12)
    ...
    # Add table for top 25 emojis (count_once), with rank first, below the plot
    ...
    plt.tight_layout()
    plt.subplots_adjust(left=0.1, right=0.9, bottom=0.35)  # Increased bottom margin
    ...
    emoji_counts_df[["emoji", "count_once", "percent_once", "unicode_code", "unicode_name"]].to_csv(
