...
from constants import DATE_TIME, PLOTS_BASE
...
class PlotManager:
    ...
    @staticmethod
    def plot_day_of_week_distribution(df: pd.DataFrame) -> None:
    ...
    @staticmethod
    def plot_bankholiday_distribution(df: pd.DataFrame) -> None:
        order = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"]
        fig, ax = plt.subplots()
        df["Day_of_Week"].value_counts().reindex(order).plot(kind="bar", ax=ax)
        ax.set_title("Accidents per Day of Week")
    ...
    @staticmethod
    def plot_rush_hour_distribution(df: pd.DataFrame) -> None:
        order = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"]
        fig, ax = plt.subplots()
        grouped = (
            df.groupby(["Day_of_Week", "Rush_Hour"]).size().unstack(fill_value=0).reindex(order)
        )
    ...
    def plot_duration_distribution(
        df: pd.DataFrame, column_name: str, title: str, filename: str
    ) -> None:
        """
        ...
        # Cut into bins
        df["Duration_Interval"] = pd.cut(
            df[column_name], bins=bins, labels=labels, right=False, ordered=True
        )
    ...
    @staticmethod
    def plot_datatype_summary_stacked_bar(df_datatype, plots_base, label="Initial") -> None:
        """
        Plots a summary of empty and None occurances per column. Closely related to Explorer.
        """
        _fig, ax = plt.subplots(figsize=(10, len(df_datatype["Header"].unique()) * 0.4))
        ...
        total = [g + b + r for g, b, r in zip(green, blue, red, strict=False)]

        green_pct = [g / t * 100 if t > 0 else 0 for g, t in zip(green, total, strict=False)]
        blue_pct = [b / t * 100 if t > 0 else 0 for b, t in zip(blue, total, strict=False)]
        red_pct = [r / t * 100 if t > 0 else 0 for r, t in zip(red, total, strict=False)]
        ...
        ax.barh(
            y_pos,
            red_pct,
            left=[g + b for g, b in zip(green_pct, blue_pct, strict=False)],
            color="red",
            label="None",
        )
    ...
    def load_shapefiles():
        ...
        """
        primary = gpd.read_file(
            "C:/Users/avtnl/Documents/HU/shapefiles/tl_2024_us_primaryroads.shp"
        )
        secondary_paths = [
    ...
    def plot_cities_basic(df: pd.DataFrame) -> None:
        ...
        secondary_roads = pd.concat([gpd.read_file(p) for p in secondary_paths]).reset_index(
            drop=True
        )
        ...
        for city in cities_to_plot:
            ...
            if not roads_clipped.empty:
                roads_clipped.plot(
                    ax=ax, linewidth=1, edgecolor="black", transform=ccrs.PlateCarree()
                )

            secondary_clipped = secondary_roads.cx[lng_min:lng_max, lat_min:lat_max]
            if not secondary_clipped.empty:
                secondary_clipped.plot(
                    ax=ax, linewidth=0.5, edgecolor="gray", alpha=0.5, transform=ccrs.PlateCarree()
                )
    ...
    def plot_cities_compared(df: pd.DataFrame) -> None:
        ...
        df["Start_Lng"] = pd.to_numeric(df["Start_Lng"], errors="coerce")
        df = df[df["Start_Lat"].between(25, 36) & df["Start_Lng"].between(-107, -93)]
        df = df[(not df["Street_in_Description"]) & (df["Primary_Structure"] == "on")]
        ...
        for city in cities_to_plot:
            ...
            df_custom = df_city[
                df_city["Location"].notna() & (df_city["Location"].str.strip() != "")
            ]

            cond_green = (df_custom["Speed_Type_Location"] == "Low Speed") & (
                df_custom["Speed_Type_Street"] == "High Speed"
            )
            cond_red = (df_custom["Speed_Type_Location"] == "High Speed") & (
    ...
    @staticmethod
    def save_plot(fig, title: str, filename: str) -> None:
        """
        ...
        print(
            f"[{datetime.now().strftime('%H:%M:%S')}][PlotManager] Plot saved as: {save_path.as_posix()}"
        )
    ...
    def plot_tasks(df: pd.DataFrame) -> None:
        ...
        PlotManager.plot_duration_distribution(
            df,
            "Duration_Start_to_End(min)",
            "Duration Start to End",
            "Duration_Start_to_End_Distribution",
        )
        PlotManager.plot_duration_distribution(
            df,
            "Duration_Start_to_Timestamp(min)",
            "Duration to Start to Weather_Timestamp",
            "Duration_Start_to_Weather_Timstamp_Distribution",
        )
