...
from constants import BANKHOLIDAYS_SET, DAY_NIGHT_COLUMNS, DAY_TIMES, WEATHER_COLUMNS
...
class DataPreparation:
    ...
    def compute_rush_hour_flag(df: pd.DataFrame) -> pd.Series:
        ...
        is_morning_rush = df["Start_Time_Only"].between(
            morning_start, morning_end, inclusive="left"
        )
        is_evening_rush = df["Start_Time_Only"].between(
            evening_start, evening_end, inclusive="left"
        )
    ...
    def compute_bankholiday_flag(df: pd.DataFrame) -> pd.Series:
        ...
        mask_friday = df["Start_Date_Only"].isin(friday_before) & (
            df["Start_Time_Only"] >= time_after_14
        )
        ...
        mask_thursday = df["Start_Date_Only"].isin(thursday_before) & (
            df["Start_Time_Only"] >= time_after_14
        )
        mask_saturday = df["Start_Date_Only"].isin(saturday_after)
        mask_sunday = df["Start_Date_Only"].isin(sunday_after)
        mask_monday = df["Start_Date_Only"].isin(monday_after) & (
            df["Start_Time_Only"] <= time_before_10
        )
    ...
    def basic_validation(
        self, df: pd.DataFrame, list_of_columns: list
        ...
        if missing_columns:
            print(
                "[DataPreparation] WARNING: The following columns do NOT exist in master file and will be skipped:"
            )
    ...
    def extract_added_columns(
        self, df: pd.DataFrame, original_number_of_columns: int
    ) -> pd.DataFrame:
        """
    ...
    def clean_using_re(
        self, df: pd.DataFrame, list_of_columns: list, cleaning_function, overwrite: bool
    ) -> pd.DataFrame:
        """
        ...
        """
        print(
            f"[{datetime.now().strftime('%H:%M:%S')}][DataPreparation] Starting cleaning using Regular Expression for multiple column"
        )
        ...
        for col in existing_columns:
            print(
                f"[{datetime.now().strftime('%H:%M:%S')}][DataPreparation] Processing column: '{col}'"
            )
            cleaned_series = []

            for _idx, val in df[col].items():
        ...
        print(
            f"[{datetime.now().strftime('%H:%M:%S')}][DataPreparation] Cleaning complete;  returning: {num_cols} columns"
        )
    ...
    def clean_conform_dictionary(
        ...
        list_of_words: dict[str, dict[str, str | list[str]]],
        action: str,
        overwrite: bool,
        ...
        if action not in allowed_actions:
            print(
                f"[DataPreparation] WARNING: Unknown action '{action}'. Supported actions are: {allowed_actions}"
            )
        ...
        print(
            f"[{datetime.now().strftime('%H:%M:%S')}][DataPreparation] Starting cleaning using dictionary for multiple columns"
        )
    ...
    def fix_and_split_column(
        ...
        df: pd.DataFrame,
        column_name: str,
        list_of_words: dict[str, dict[str, str | list]],
        overwrite: bool,
    ) -> pd.DataFrame:
        """
        ...
        print(
            f"[{datetime.now().strftime('%H:%M:%S')}][DataPreparation] Start splitting string in column '{column_name}' on indicated position"
        )

        direction_aliases = {"n", "e", "s", "w", "ne", "nw", "se", "sw", "nb", "sb", "eb", "wb"}
        target_number = (
            2  # Split will only apply to rows where {column_name}_count == target_number
        )
        ...
        if count_col not in df.columns:
            raise KeyError(
                f"Expected column '{count_col}' not found in DataFrame. Did you run preseding scripts?"
            )

        part1_series = []
        part2_series = []

        for text, count in zip(df[column_name], df[count_col], strict=False):
    ...
    def merge_parts_of_column(
        self, df: pd.DataFrame, column_name: str, suffix: str, overwrite: bool
    ) -> pd.DataFrame:
        """
        ...
        """
        print(
            f"[{datetime.now().strftime('%H:%M:%S')}][DataPreparation] Starting merge of relevant columns"
        )
        ...
        for part1, part2 in zip(df[part1_col], df[cleaned_part2_col], strict=False):
            # Handle None or NaN
    ...
    def normalize_road_number(
        self, df: pd.DataFrame, list_of_columns: list, overwrite: bool
    ) -> pd.DataFrame:
        ...
        print(
            f"[{datetime.now().strftime('%H:%M:%S')}][DataPreparation] Normalizing road numbers for multiple columns"
        )
        ...
        if not existing_columns:
            print(
                "[DataPreparation] WARNING: No valid columns to process. Returning original DataFrame."
            )
        ...
        for col in existing_columns:
            ...
            if not numbered_road_col:
                ...
                numbered_road_col = [
                    ...
                    for existing, new in zip(numbered_road_col, temp_numbered_roads, strict=False)
                ]
    ...
    def add_road_details(
        ...
        road_dict: dict[str, dict[str, str | list[str]]],
        overwrite: bool,
    ) -> pd.DataFrame:
        """
        ...
        """
        print(
            f"[{datetime.now().strftime('%H:%M:%S')}][DataPreparation] Adding road details to one or more columns"
        )
        ...
        for col in existing_cols:
            ...
            for val in df[col].fillna("").astype(str):
                ...
                for _key, props in road_dict.items():
    ...
    def compute_distances_and_generate_stats(
        ...
        """
        print(
            f"[{datetime.now().strftime('%H:%M:%S')}][DataPreparation] Starting examining distances"
        )
        ...
        for i, row in df.iterrows():
            if (
                pd.notna(row["Start_Lat"])
                and pd.notna(row["Start_Lng"])
                and pd.notna(row["End_Lat"])
                and pd.notna(row["End_Lng"])
            ):
        ...
        print(
            f"[{datetime.now().strftime('%H:%M:%S')}][DataPreparation] Examining distances now complete"
        )
    ...
    def find_missing_data_weather(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        print(
            f"[{datetime.now().strftime('%H:%M:%S')}][DataPreparation] Starting search for missing 'Weather' data"
        )
    ...
    def find_missing_data_period_of_day(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        print(
            f"[{datetime.now().strftime('%H:%M:%S')}][DataPreparation] Starting search for missing 'Period_of_Day' data"
        )
        ...
        # === Preprocessing ===
        df_valid["Start_Timestamp"] = (
            df_valid["Start_Time"].values.astype("datetime64[s]").astype("int64")
        )
        df_missing["Start_Timestamp"] = (
            df_missing["Start_Time"].values.astype("datetime64[s]").astype("int64")
        )
    ...
    def find_missing_data_period_of_day_from_table(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        print(
            f"[{datetime.now().strftime('%H:%M:%S')}][DataPreparation] Starting sunrise/sunset table lookup"
        )
        ...
        for i, row in df.iterrows():
            if pd.isna(row["Sunrise_Sunset"]):
                ...
                if used_table:
                    updated_info = self.update_modification_notes(
                        row["Modification_Notes"], "From table"
                    )
    ...
    def retrieve_best_matches(
        self, df: pd.DataFrame, df_ids: pd.DataFrame, data_category: str
    ) -> pd.DataFrame:
        """
        ...
        print(
            f"[{datetime.now().strftime('%H:%M:%S')}][DataPreparation] Starting data retrieval for category: {data_category}"
        )

        # Validate category
        valid_categories = {
            "Weather": {"columns": WEATHER_COLUMNS, "id_column": "Matching_ID"},
            "Day/Night": {"columns": DAY_NIGHT_COLUMNS, "id_column": "Matching_ID"},
        }

        if data_category not in valid_categories:
            raise ValueError(
                f"[ERROR] Invalid data_category '{data_category}'. Must be 'Weather' or 'Day/Night'."
            )
        ...
        for idx in df_ids_copy.index:
            ...
            df.at[idx, "Modification_Notes"] = self.update_modification_notes(
                existing_note, new_note
            )

        print(
            f"[{datetime.now().strftime('%H:%M:%S')}][DataPreparation] Retrieval complete for category: {data_category}"
        )
    ...
    def summarize_period_of_day(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        print(
            f"[{datetime.now().strftime('%H:%M:%S')}][DataPreparation] Generating summarized 'Period_of_Day'"
        )
    ...
    def complete_period_of_day_from_table(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for idx in df[mask_missing].index:
            existing_note = df.at[idx, "Modification_Notes"]
            df.at[idx, "Modification_Notes"] = self.update_modification_notes(
                existing_note, "Period_of_Day from table"
            )
    ...
    def change_weather_condition(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        print(
            f"[{datetime.now().strftime('%H:%M:%S')}][DataPreparation] Updating 'Weather_Condition'"
        )
        ...
        mask_vicinity = (df["Rainy"]) & (df["Precipitation(in)"] == 0)
        df.loc[mask_vicinity, "Weather_Condition"] = df.loc[
            mask_vicinity, "Weather_Condition"
        ].apply(insert_using_re)

        for idx in df[mask_vicinity].index:
            existing_note = df.at[idx, "Modification_Notes"]
            df.at[idx, "Modification_Notes"] = self.update_modification_notes(
                existing_note, 'Added "vicinity" to WC'
            )

        # Case 2: Rainy == False and Precipitation > 0
        mask_airport_rain = (not df["Rainy"]) & (df["Precipitation(in)"] > 0)
        ...
        for idx in df[mask_airport_rain].index:
            ...
            existing_note = df.at[idx, "Modification_Notes"]
            df.at[idx, "Modification_Notes"] = self.update_modification_notes(
                existing_note, 'Added "rain at airport" to WC'
            )
