...
editor = DataEditor(
    file_manager, data_preparation
)  # When instantiating the editor, we tell thje editor which instance of DataPreparation to use
...
if "script_2" in scripts:
    ...
    df = file_manager.read_csv(
        "US_Accidents_TX_subset_amended_17Jun2025_0945.csv"
    )  # For script_2 | Please change to correct file_name
    print("Reading input CSV...")
    Explorer.final(df, file_manager, PLOTS_BASE)
    df = editor.script_2(df)
    df_structures = file_manager.read_csv(
        "Structures_Extracted_Details_17Jun2025_0947.csv"
    )  # TX-subset
...
if "script_3" in scripts:
    ...
    df = file_manager.read_csv(
        "US_Accidents_TX_subset_amended_17Jun2025_0949.csv"
    )  # For script_3 | Please change to correct file_name
    print("Reading input CSV...")
    Explorer.initial_exploration(df, file_manager, PLOTS_BASE)
...
if "script_4" in scripts:
    ...
    df = file_manager.read_csv(
        "US_Accidents_TX_subset_amended_17Jun2025_0950.csv"
    )  # For script_4 | Please change to correct file_name
    print("Reading input CSV...")
    Explorer.final(df, file_manager, PLOTS_BASE)
...
if "script_5" in scripts:
    df = file_manager.read_csv(
        "US_Accidents_TX_subset_amended_17Jun2025_1037.csv"
    )  # For script_5 | Please change to correct file_name
