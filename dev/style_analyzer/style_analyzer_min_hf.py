...
# ------------------------------------------------------------------
def compute_embeddings(df: pd.DataFrame, model_name: str = "AnnaWegmann/Style-Embedding") -> pd.DataFrame:
    ...
    embeddings = model.encode(messages, batch_size=32, show_progress_bar=True, convert_to_numpy=True)
...
def plot_tsne_individual(agg: pd.DataFrame, out_dir: Path, draw_ellipses: bool = True):
    ...
    fig = px.scatter(
        agg, x='tsne_x', y='tsne_y', color=Columns.AUTHOR.value, size='msg_count',
        hover_data={'style_words_per_msg': ':.1f', 'style_emoji_per_msg': ':.3f', 'style_name': True, 'msg_count': True},
...
def plot_tsne_group(agg: pd.DataFrame, out_dir: Path, draw_ellipses: bool = True):
    ...
    group_colors = {
        ...
        "tillies": "#808080", "AvT": "#d62728"
    }

    fig = px.scatter(
        agg, x="tsne_x", y="tsne_y", color="plot_group", size="msg_count",
...
def run_style_analysis(
    ...
    use_embeddings: bool = False,        # ← Toggle HF
    hybrid_features: bool = False,       # ← Combine
