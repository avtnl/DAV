...
def aggregate_basic(df: pd.DataFrame, group_cols: list[str], by_year: bool) -> pd.DataFrame:
    ...
    # ---- AGGREGATE ----
    agg = df.groupby(full_group_cols).agg(
    ...
    burst_defs = [
        ...
        (Columns.LIST_OF_CONNECTED_EMOJIS.value, {
            ...
            '🤣🤣': 'laugh_2', '🤣🤣🤣': 'laugh_3plus',
            '😄😄': 'happy_2',
            'thumbs_up': 'thumbs_2', 'thumbs_up_light_skin_tone': 'thumbs_2', 'thumbs_up_medium_light_skin_tone': 'thumbs_2',
    ...
    # ---- DEBUG: PRINT AvT ----
    print("\n=== DEBUG: AvT in agg ===")
    print(agg[agg["whatsapp_group_temp"] == "AvT"][["author", "year", "msg_count", "whatsapp_group_temp"]])
    print("========================\n")
...
def run_style_analysis(
    ...
    if by_year:
        ...
        if not (agg[Columns.AUTHOR.value] == avt_author).any():
            zero_row = pd.DataFrame([{
                Columns.AUTHOR.value: avt_author,
                'msg_count': 0,
                'whatsapp_group_temp': 'AvT'
