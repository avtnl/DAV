# --------------------------------------------------------------
# style_analyzer_gen31.py  (FIXED – NO KeyError, VECTORIZED pct_replies)
# --------------------------------------------------------------
...
def load_enriched() -> pd.DataFrame:
    ...
    # ---- ADD TEMP COLUMN: whatsapp_group_temp ----
    df["whatsapp_group_temp"] = df[Columns.WHATSAPP_GROUP.value]
    df.loc[df[Columns.AUTHOR.value] == "Anthony van Tilburg", "whatsapp_group_temp"] = "AvT"
...
# ------------------------------------------------------------------
# 3. Core aggregation – AvT RED, FULLY WORKING
# ------------------------------------------------------------------
def aggregate_basic(df: pd.DataFrame, group_cols: list[str], by_year: bool) -> pd.DataFrame:
    # ---- CREATE whatsapp_group_temp FIRST ----
    df["whatsapp_group_temp"] = df[Columns.WHATSAPP_GROUP.value].copy()
    df.loc[df[Columns.AUTHOR.value] == "Anthony van Tilburg", "whatsapp_group_temp"] = "AvT"

    # ---- FULL GROUP COLS (includes whatsapp_group_temp) ----
    full_group_cols = [*group_cols, "whatsapp_group_temp"]

    # ---- AGGREGATE ----
    agg = (
        df.groupby(full_group_cols)
        .agg(
    ...
    burst_defs = [
        ...
        (
            Columns.LIST_OF_CONNECTED_EMOJIS.value,
            {
                ...
                "😄😄": "happy_2",
                "thumbs_up": "thumbs_2",
                "thumbs_up_light_skin_tone": "thumbs_2",
                "thumbs_up_medium_light_skin_tone": "thumbs_2",
                "red_heart": "heart_single",
            },
        ),
        (
    ...
    print("\n=== DEBUG: AvT in agg ===")
    print(
        agg[agg["whatsapp_group_temp"] == "AvT"][
            ["author", "year", "msg_count", "whatsapp_group_temp"]
        ]
    )
    print("========================\n")
...
# 8. Individual t-SNE
# ------------------------------------------------------------------
def plot_tsne_individual(agg: pd.DataFrame, out_dir: Path) -> None:
    ...
    author_color_map = {
        auth: colors[i % len(colors)] for i, auth in enumerate(agg[Columns.AUTHOR.value].unique())
    }
...
# ------------------------------------------------------------------
# 9. Group-level t-SNE – AvT RED (NO ELLIPSIS)
# ------------------------------------------------------------------
def plot_tsne_group(agg: pd.DataFrame, out_dir: Path) -> None:
    out_dir.mkdir(parents=True, exist_ok=True)

    agg["plot_group"] = agg["whatsapp_group_temp"]
    ...
    fig = px.scatter(
        ...
        hover_data={
            ...
            "whatsapp_group_temp": True,
        },
        title="Linguistic Style Clusters (t-SNE) – 5 Groups (AvT RED)",
        width=950,
        height=650,
    )
    fig.update_traces(marker={"opacity": 0.85, "line": {"width": 1, "color": "black"}})
...
def run_style_analysis(
    ...
    agg = aggregate_basic(df, group_cols, by_year)

    # --- DEBUG: PRINT AvT ROWS ---
    print("\n=== DEBUG: AvT in agg ===")
    avt_debug = agg[agg["whatsapp_group_temp"] == "AvT"]
    print(avt_debug[["author", "year", "msg_count", "whatsapp_group_temp"]])
    print("========================\n")
    # ------------------------------

    # --- FORCE AvT ROW IF MISSING ---
    avt_author = "Anthony van Tilburg"
    if by_year:
        years = df[Columns.YEAR.value].unique()
        for year in years:
            mask = (agg[Columns.AUTHOR.value] == avt_author) & (agg[Columns.YEAR.value] == year)
            if not mask.any():
                zero_row = pd.DataFrame(
                    [
                        {
                            Columns.AUTHOR.value: avt_author,
                            Columns.YEAR.value: year,
                            "msg_count": 0,
                            "whatsapp_group_temp": "AvT",
                        }
                    ]
                )
                for col in agg.columns:
                    if col not in zero_row.columns:
                        zero_row[col] = 0
                agg = pd.concat([agg, zero_row], ignore_index=True)
    elif not (agg[Columns.AUTHOR.value] == avt_author).any():
        zero_row = pd.DataFrame(
            [{Columns.AUTHOR.value: avt_author, "msg_count": 0, "whatsapp_group_temp": "AvT"}]
        )
        for col in agg.columns:
            if col not in zero_row.columns:
                zero_row[col] = 0
...
if __name__ == "__main__":
    agg = run_style_analysis(by_year=True, by_group=True)  # 5 groups
    # agg = run_style_analysis(by_year=True, by_group=False)  # individual
