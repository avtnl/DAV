...
def aggregate_basic(df: pd.DataFrame, group_cols: list[str]) -> pd.DataFrame:
    ...
    def compute_pct_replies(g):
        return (
            g[Columns.PREVIOUS_AUTHOR.value].notna()
            & (g[Columns.PREVIOUS_AUTHOR.value] != g[Columns.AUTHOR.value])
        ).mean()
    ...
    burst_defs = [
        ...
        (Columns.LIST_OF_CONNECTED_EMOJIS.value, {None: "emoji_bursts"}),  # any sequence → count
        (Columns.LIST_OF_CONNECTED_CAPITALS.value, {None: "caps_bursts"}),  # any all-caps burst
    ]
...
def run_style_analysis(
    by_year: bool = True, do_clustering: bool = True, out_dir: Path = Path("style_output")
):
    df = load_enriched()
...
if __name__ == "__main__":
    agg = run_style_analysis(by_year=True, do_clustering=True, out_dir=Path("style_output"))
