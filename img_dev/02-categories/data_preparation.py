...
class DataPreparation:
    def build_visual_categories(self, df):
        ...
        try:
            ...
            active_years_period = (
                filter_df.groupby(["whatsapp_group", "author"])["year"]
                .agg(["min", "max"])
                .reset_index()
            )
            ...
            group_total = (
                filter_df.groupby("whatsapp_group").size().reset_index(name="total_messages")
            )
            ...
            non_anthony_counts = (
                non_anthony.groupby(["whatsapp_group", "author"])
                .size()
            ...
            anthony_group = (
                anthony.groupby("whatsapp_group").size().reset_index(name="anthony_messages")
            )
            anthony_group = (
    ...
    def build_visual_time(self, df):
        ...
        try:
            ...
            logger.info(
                f"Average message counts per week (excluding 2020):\n{average_no_2020.head()}"
            )
            ...
            weeks_1_12_35_53_no_2020 = average_no_2020[
                (average_no_2020["isoweek"].between(1, 12))
                | (average_no_2020["isoweek"].between(35, 53))
            ...
            logger.info(
                f"Average message count (excl. 2020) for weeks 12-19: {weeks_12_19_no_2020:.2f}"
            )
            logger.info(
                f"Average message count (excl. 2020) for weeks 19-35: {weeks_19_35_no_2020:.2f}"
            )
        ...
        try:
            ...
            for message in emoji_msgs["message"]:
                ...
                emojis = [
                    char
                    for char in message
                    if char in emoji.EMOJI_DATA and char not in ignore_emojis
                ]
